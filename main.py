...
NEW_API_ADMIN_TOKEN = os.getenv("NEW_API_ADMIN_TOKEN", "")  # 管理员token，用于创建兑换码
DATABASE_URL = os.getenv("DATABASE_URL", "sqlite:///./coupon.db")
CLAIM_COOLDOWN_HOURS = int(os.getenv("CLAIM_COOLDOWN_HOURS", "8"))
COUPON_QUOTA = int(os.getenv("COUPON_QUOTA", "500000"))  # 每次领取的额度（New API的quota单位）
...
async def get_current_user(request: Request) -> dict:
    ...
    # 1. 从 cookie 获取 (New API 使用 session cookie)
...
async def claim_coupon(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # 记录领取
    expire_time = now + timedelta(hours=24)  # 24小时有效期
    record = ClaimRecord(
        user_id=user_id,
        username=username,
...
async def login_redirect(redirect: str = "/"):
    """跳转到 New API 登录"""
    return RedirectResponse(url=f"{NEW_API_URL}/login")

def get_claim_page_html():
    """返回领取页面 HTML"""
...
<body class="bg-gray-50 min-h-screen">
    ...
    <nav class="gradient-header text-white py-4 px-6 shadow-lg">
        <div class="container mx-auto flex justify-between items-center">
            ...
            <div id="nav-right" class="flex items-center space-x-4">
                ...
                <button id="logout-btn" onclick="logout()" class="hidden hover:text-gray-200 transition">
                    🚪 注销
                </button>
            </div>
        </div>
    ...
    <main class="container mx-auto px-4 py-8 max-w-3xl">
        ...
        <div id="login-prompt" class="card p-8 text-center hidden">
            ...
            <a href="{NEW_API_URL}/login" 
               class="inline-block bg-blue-500 hover:bg-blue-600 text-white px-8 py-3 rounded-lg font-semibold transition">
                前往登录
            </a>
        </div>
        ...
        <div id="main-content" class="hidden">
            ...
            <div class="card p-6 mb-6">
                ...
                <div class="text-center py-8">
                    <button id="claim-btn" onclick="claimCoupon()" 
                            class="btn-claim text-white py-4 px-12 rounded-xl text-lg font-bold shadow-lg">
            ...
            <div class="card p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button onclick="loadStatus()" class="text-blue-500 hover:text-blue-600 text-sm">
                        🔄 刷新
                    </button>
                </div>
    ...
    <script>
        ...
        function showLoggedIn() {{
            ...
            document.getElementById('welcome-msg').textContent = 
                `欢迎您，${{currentUser.display_name || currentUser.username}} (ID: ${{currentUser.id}})`;
        }}
